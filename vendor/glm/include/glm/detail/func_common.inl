...
{
	template<typename T>
	GLM_FUNC_QUALIFIER T round_scalar(T x)
	{
		return std::round(x);
	}

	template<typename T>
	GLM_FUNC_QUALIFIER T trunc_scalar(T x)
	{
		return std::trunc(x);
	}
...
#	endif
	...
	struct compute_trunc
	{
		GLM_FUNC_QUALIFIER static vec<L, T, Q> call(vec<L, T, Q> const& x)
		{
			return detail::functor1<vec, L, T, T, Q>::call(trunc_scalar<T>, x);
		}
	};
	...
	struct compute_round
	{
		GLM_FUNC_QUALIFIER static vec<L, T, Q> call(vec<L, T, Q> const& x)
		{
			return detail::functor1<vec, L, T, T, Q>::call(round_scalar<T>, x);
		}
	};
